			...
			const onPointerMove = (e: PointerEvent) => {
				...
				for (const track of tracks) {
					...
					for (const itemId of track.items) {
						...
						const itemX = canvasLeft + item.left * canvasTransformation.scale;
						const itemY = canvasTop + item.top * canvasTransformation.scale;
						const itemEndX = item.width * canvasTransformation.scale + itemX;
						const itemEndY = item.height * canvasTransformation.scale + itemY;
